...
use rgpui::{AnyWindowHandle, AppContext, ClickEvent, Global, SharedString, Window};
...
type ClickCb = Arc<dyn Fn(&Notification, &ClickEvent, &mut Window, &mut rgpui::App)>;
type DismissCb = Arc<dyn Fn(&Notification, &mut Window, &mut rgpui::App)>;
...
struct State {
    ...
    persisted_state: Option<rgpui::Entity<PersistedState>>,
...
impl NotificationCenter {
    ...
    pub fn notify(&self, n: Notification, cx: &mut rgpui::App) -> Uuid {
        let id = n.id;
    ...
    pub fn dismiss(&self, id: Uuid, cx: &mut rgpui::App) {
        {
            let mut state = self.state.lock().unwrap();
    ...
    pub fn clear(&self, cx: &mut rgpui::App) {
        {
            let mut state = self.state.lock().unwrap();
            state.queue.clear();
    ...
    pub(crate) fn dismiss_from_ui(&self, id: Uuid, window: &mut Window, cx: &mut rgpui::App) {
        let (n, cb) = {
            let state = self.state.lock().unwrap();
            let n = state.queue.iter().find(|n| n.id == id).cloned();
    ...
    pub fn load_persisted(&self, cx: &mut rgpui::App) {
        {
            let mut state = self.state.lock().unwrap();
    ...
    pub(crate) fn bind_persisted_state(
        &self,
        entity: rgpui::Entity<PersistedState>,
        cx: &mut rgpui::App,
    ) {
    ...
    fn maybe_schedule_auto_dismiss(&self, id: Uuid, cx: &mut rgpui::App) {
    ...
    fn refresh_host(&self, cx: &mut rgpui::App) {
    ...
    fn persist(&self, cx: &mut rgpui::App) {
