...
impl NotificationHost {
    ...
    pub fn max_width(mut self, width: rgpui::Pixels) -> Self {
        self.max_width = width;
        self
    }

    /// Offset from the top-right corner.
    pub fn offset(mut self, offset: rgpui::Pixels) -> Self {
        self.offset = offset;
        self
    }
}
    ...
    fn render(self, window: &mut Window, cx: &mut App) -> impl IntoElement {
        ...
        if persist_enabled {
            ...
            let persisted = window.use_keyed_state(
                (
                    rgpui::ElementId::from("ui:notification-center"),
                    persist_key,
                ),
                cx,
