...
use rgpui::{Div, ElementId, Hsla, InteractiveElement, Pixels, Stateful, Styled};
...
impl AnimateExt for Div {
    ...
    fn animate_slide(
        ...
        let (ml, mt) = match direction {
            SlideDirection::Left => (rgpui::px(distance_f * (progress - 1.0)), rgpui::px(0.0)),
            SlideDirection::Right => (rgpui::px(distance_f * (1.0 - progress)), rgpui::px(0.0)),
            SlideDirection::Up => (rgpui::px(0.0), rgpui::px(distance_f * (progress - 1.0))),
            SlideDirection::Down => (rgpui::px(0.0), rgpui::px(distance_f * (1.0 - progress))),
        };
