...
use rgpui::{AnyElement, IntoElement, ParentElement, Styled, div, px};
use rgpui_yororen_ui::component::{IconName, checkbox, icon_button, label, list_item, tag};
use rgpui_yororen_ui::i18n::Translate;
...
impl TodoItem {
    ...
    pub fn render(cx: &rgpui::App, todo: &Todo, compact_mode: bool) -> AnyElement {
        if compact_mode {
    ...
    fn render_compact(cx: &rgpui::App, todo: &Todo) -> impl IntoElement {
        let todo_id = todo.id;
    ...
    fn render_normal(cx: &rgpui::App, todo: &Todo) -> impl IntoElement {
        let todo_id = todo.id;
        let title = todo.title.clone();
        let category_label = cx.t(todo.category.key());
        let completed = todo.completed;
