...
type ChangeFn = Arc<dyn Fn(String, &ClickEvent, &mut rgpui::Window, &mut rgpui::App)>;
...
pub struct ComboBox {
    ...
    menu_width: Option<rgpui::Pixels>,
    max_results: usize,
...
impl ComboBox {
    ...
    where
        F: 'static + Fn(String, &ClickEvent, &mut rgpui::Window, &mut rgpui::App),
    {
    ...
    pub fn menu_width(mut self, width: rgpui::Pixels) -> Self {
        self.menu_width = Some(width);
        self
    }

    pub fn min_menu_width(mut self, width: rgpui::Pixels) -> Self {
        self.menu_width = Some(width);
        self
    }
}
...
impl RenderOnce for ComboBox {
    fn render(self, window: &mut rgpui::Window, cx: &mut rgpui::App) -> impl IntoElement {
        ...
        let trigger = trigger.when(is_open, move |this| {
            ...
            this.child(rgpui::deferred(animated_menu).with_priority(100))
        });
