...
use rgpui::{App, Context, FocusHandle, ParentElement, SharedString, UTF16Selection};

use crate::component::TextEditState;
use crate::constants::CURSOR_BLINK_INTERVAL;

pub type TextAreaHandler = Arc<dyn Fn(SharedString, &mut rgpui::Window, &mut App)>;
...
pub struct TextAreaState {
    ...
    pub scroll_x: rgpui::Pixels,
    pub scroll_y: rgpui::Pixels,
    pub last_layout: Option<super::layout::TextAreaLayout>,
    pub last_bounds: Option<rgpui::Bounds<rgpui::Pixels>>,
    pub is_selecting: bool,
    pub cursor_visible: bool,
    pub cursor_blink_epoch: usize,
    pub focus_subscription: Option<rgpui::Subscription>,
    pub preferred_x: Option<rgpui::Pixels>,
    pub wrap: WrapMode,
...
impl TextAreaState {
    pub fn new(cx: &mut App) -> Self {
        Self {
            ...
            placeholder: "".into(),
            scroll_x: rgpui::Pixels::ZERO,
            scroll_y: rgpui::Pixels::ZERO,
            last_layout: None,
            last_bounds: None,
            is_selecting: false,
            cursor_visible: true,
    ...
    pub fn set_content(&mut self, content: impl Into<SharedString>) {
        ...
        self.scroll_x = rgpui::Pixels::ZERO;
        self.scroll_y = rgpui::Pixels::ZERO;
        self.preferred_x = None;
    }

    pub fn scroll_x(&self) -> rgpui::Pixels {
        self.scroll_x
    }

    pub fn scroll_y(&self) -> rgpui::Pixels {
        self.scroll_y
    }
    ...
    pub fn reset_cursor_blink(&mut self, window: &mut rgpui::Window, cx: &mut Context<Self>) {
        self.show_cursor(cx);
    ...
    pub fn focus_in(&mut self, window: &mut rgpui::Window, cx: &mut Context<Self>) {
    ...
    pub fn move_to(&mut self, offset: usize, window: &mut rgpui::Window, cx: &mut Context<Self>) {
        self.preferred_x = None;
    ...
    pub fn select_to(&mut self, offset: usize, window: &mut rgpui::Window, cx: &mut Context<Self>) {
        self.reset_cursor_blink(window, cx);
    ...
    pub fn index_for_mouse_position(&self, position: rgpui::Point<rgpui::Pixels>) -> usize {
        ...
        if local_y < rgpui::Pixels::ZERO {
            local_y = rgpui::Pixels::ZERO;
        }
        if local_x < rgpui::Pixels::ZERO {
            local_x = rgpui::Pixels::ZERO;
        }
        let row = layout
            .row_for_y(local_y)
...
impl rgpui::RenderOnce for TextAreaState {
    fn render(self, _: &mut rgpui::Window, _: &mut App) -> impl rgpui::IntoElement {
        rgpui::div().child(self.edit.content().clone())
    }
}

impl rgpui::EntityInputHandler for TextAreaState {
    fn text_for_range(
        &mut self,
    ...
    fn unmark_text(&mut self, _: &mut rgpui::Window, _: &mut Context<Self>) {
    ...
    fn bounds_for_range(
        ...
        _: &mut rgpui::Window,
        _: &mut Context<Self>,
    ) -> Option<rgpui::Bounds<rgpui::Pixels>> {
        ...
        Some(rgpui::Bounds::new(
            rgpui::point(
                bounds.left() + x - self.scroll_x,
                bounds.top() + y - self.scroll_y,
            ),
            rgpui::size(rgpui::px(2.), layout.line_height),
        ))
    }

    fn character_index_for_point(
        &mut self,
        point: rgpui::Point<rgpui::Pixels>,
        _: &mut rgpui::Window,
        _: &mut Context<Self>,
    ) -> Option<usize> {
        if self.edit.content().is_empty() {
...
impl rgpui::Focusable for TextAreaState {
