...
type DragMoveFn = Box<dyn Fn(&MouseMoveEvent, &mut rgpui::Window, &mut rgpui::App)>;

type DragStartFn = Box<dyn Fn(&MouseDownEvent, &mut rgpui::Window, &mut rgpui::App)>;

type DragEndFn = Box<dyn Fn(&MouseMoveEvent, &mut rgpui::Window, &mut rgpui::App)>;
...
impl DragHandle {
    ...
    where
        F: 'static + Fn(&MouseMoveEvent, &mut rgpui::Window, &mut rgpui::App),
    {
