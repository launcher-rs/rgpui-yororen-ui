...
type ModalCloseCallback = Box<dyn Fn(&mut rgpui::Window, &mut rgpui::App)>;
...
pub struct Modal {
    ...
    actions: Option<rgpui::AnyElement>,
    width: rgpui::Pixels,
    bg: Option<Hsla>,
    border: Option<Hsla>,
    closable: bool,
...
impl RenderOnce for Modal {
    fn render(self, _window: &mut rgpui::Window, cx: &mut rgpui::App) -> impl IntoElement {
        ...
        let mut header_children: Vec<rgpui::AnyElement> = vec![];
...
pub fn modal_actions_row(
    children: impl IntoIterator<Item = rgpui::AnyElement>,
) -> impl IntoElement {
    div()
        .flex()
        .items_center()
