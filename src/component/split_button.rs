...
type ClickFn = Box<dyn Fn(&ClickEvent, &mut rgpui::Window, &mut rgpui::App)>;
type ActionFn = Arc<dyn Fn(SplitButtonAction, &ClickEvent, &mut rgpui::Window, &mut rgpui::App)>;
...
impl SplitButton {
    ...
    where
        F: 'static + Fn(SplitButtonAction, &ClickEvent, &mut rgpui::Window, &mut rgpui::App),
    {
