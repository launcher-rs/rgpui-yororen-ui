...
impl rgpui::InteractiveElement for PasswordInput {
...
impl RenderOnce for PasswordInput {
    fn render(self, window: &mut rgpui::Window, cx: &mut App) -> impl rgpui::IntoElement {
        ...
        let height = self.height.unwrap_or_else(|| rgpui::px(36.).into());
        let inset = if disabled {
            rgpui::px(6.)
        } else {
            rgpui::px(5.)
        };
