...
type RenderRowFn =
    Box<dyn FnMut(usize, &mut rgpui::Window, &mut rgpui::App) -> AnyElement + 'static>;

/// Controller for a [`VirtualList`].
///
/// This is intentionally a thin wrapper over `rgpui::ListState` so Yororen UI users
/// don't have to call `reset/splice/scroll_to_reveal_item` directly.
#[derive(Clone, Debug)]
pub struct VirtualListController {
    state: rgpui::ListState,
}

impl VirtualListController {
    pub fn new(state: rgpui::ListState) -> Self {
        Self { state }
    }

    pub fn state(&self) -> rgpui::ListState {
        self.state.clone()
    }
...
/// Widget: a virtualized list based on `rgpui::list`.
///
...
///
/// State ownership:
/// - The underlying `rgpui::ListState` must be held by the caller's view/state.
...
pub struct VirtualList {
    ...
    style: rgpui::StyleRefinement,
}

impl VirtualList {
    pub fn new(
        state: rgpui::ListState,
        render_row: impl FnMut(usize, &mut rgpui::Window, &mut rgpui::App) -> AnyElement + 'static,
    ) -> Self {
        Self {
            state,
            sizing_behavior: ListSizingBehavior::default(),
            render_row: Box::new(render_row),
            style: rgpui::StyleRefinement::default(),
        }
    }
...
impl RenderOnce for VirtualList {
    fn render(self, _window: &mut rgpui::Window, _cx: &mut rgpui::App) -> impl IntoElement {
        ...
        // `rgpui::List` is `Styled`, so we can transfer our style refinement onto it.
        let mut inner =
...
/// An ergonomic container that owns both `rgpui::ListState` and a [`VirtualListController`].
///
...
pub fn virtual_list_state(
    ...
    rgpui::ListState::new(item_count, alignment, overdraw)
}
